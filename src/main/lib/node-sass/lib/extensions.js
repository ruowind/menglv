...
var eol = require('os').EOL,
  ...
  path = require('path'),
  defaultBinaryPath = path.join(__filename, '../../../../../../build/node-sass');
...
function getInstalledBinaries() {
  ...
  return require('bindings')('./node-sass/binding');
}
...
function getVersionInfo(binding) {
  return [
    ['node-sass', pkg.version, '(Wrapper)', '[JavaScript]'].join('\t'), ['libsass  ', binding.libsassVersion(), '(Sass Compiler)', '[C/C++]'].join('\t'),
  ].join(eol);
}
